...
const NewTable = ({ columns, data, exportExcel = false, fileName, resetData, undoData, saveChanges }: NewTableProps) => {
    ...
    return (
        <>
            <Button color="primary" onClick={undoData} size="md" className="shadow-sm mr-1" data-tip data-for="UndoTip">
                <CornerUpLeft className="feather" size={30} style={{ marginTop: '-5px' }} />
                <span style={{ padding: '5px' }}>Undo</span>
            </Button>
            <Button color="primary" onClick={saveChanges} data-tip data-for="SaveChangeTip" className="shadow-sm mr-1">
                {/* <Filter className="feather" /> */}
                Save Changes
            </Button>
            <Button color="primary" onClick={resetData} data-tip data-for="ResetTableTip" className="shadow-sm mr-1">
            ...
            {exportExcel && (
                <>
                    <Button
                        ...
                        className="shadow-sm"
                    >
                        <Download className="feather" size={30} style={{ marginTop: '-5px' }} />
                        <span style={{ padding: '5px' }}>Export Excel</span>
                    </Button>
                </>
            )}
            <div className="react_table--wrapper" style={{ marginTop: '30px' }}>
                <table
                    {...getTableProps()}
                    // ref={react_table}
                    id="react_table"
